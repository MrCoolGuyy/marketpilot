...
class EvaluationBatchResult(BaseModel):
    ...
    observations: list[
        CandidateEvaluationObserved | CandidateRejectedObserved | CounterfactualCandidateObserved
    ]
...
class CausalPipeline:
    ...
    def __init__(
        self, pricing: PricingPolicy, validation: ValidationPolicy, economics: CausalEconomicsEngine
    ):
    ...
    def process_signals(
        ...
        for intent in intents:
            ...
            if priced.rejection_reason:
                ...
                obs = CandidateRejectedObserved(
                    ...
                    executable_entry_price=priced.executable_entry_price
                    if hasattr(priced, "executable_entry_price")
                    else None,
            ...
            # 4. Sizing
            sizing = self.economics.provisional_size(
                priced, effective_risk_capital, risk_fraction, max_risk_fraction
            )
            ...
            # 6. Build Candidate
            final = self.economics.build_final_candidate(
                priced, assessment, pre_size, sizing, size_aware
            )
            ...
            if final.rejection_reason:
                obs = CandidateRejectedObserved(
                    ...
                    direction=intent.direction,
                    signal_timestamp=intent.signal_timestamp,
                    quote_id=final.priced_candidate.quote.quote_id
                    if final.priced_candidate.quote
                    else None,
                    quote_timestamp=final.priced_candidate.quote.quote_timestamp
                    if final.priced_candidate.quote
                    else None,
                    evidence_status=final.assessment.status,
                    pricing_status=final.priced_candidate.pricing_status,
                    executable_entry_price=final.priced_candidate.executable_entry_price
                    if hasattr(final.priced_candidate, "executable_entry_price")
                    else None,
        ...
        for i, final in enumerate(candidates):
            ...
            cf = CounterfactualCandidateObserved(
                observation_id=str(uuid.uuid4()), candidate=final, rank=rank
            )
            observations.append(cf)

        return EvaluationBatchResult(candidates=candidates, observations=observations)
