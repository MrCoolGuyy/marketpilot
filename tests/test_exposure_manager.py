...
def test_snapshot_immutability():
    ...
    with pytest.raises(Exception):
        snap1.total_risk_amount = Decimal("100")
...
def test_atomic_reservation_success():
    ...
    assert snap2.reserved_risk_amount == Decimal("10")
...
def test_replace_all():
    ...
    assert snap.active_risk_amount == Decimal("50")
...
def test_apply_confirmed_transition():
    ...
    # Transition
    em.apply_confirmed_transition("alloc-1", "pos-1", Decimal("10"), Decimal("10"))

    snap2 = em.snapshot()
    assert "alloc-1" not in snap2.reserved_allocation_ids
    assert "pos-1" in snap2.active_position_ids
    assert snap2.active_risk_amount == Decimal("10")
    assert snap2.reserved_risk_amount == Decimal("0")
...
def test_concurrent_state_access():
    ...
    assert snap.total_risk_amount == Decimal(str(len(snap.reserved_allocation_ids)))
